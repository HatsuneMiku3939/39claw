...
- a journal thread reset with durable memory carried through `AGENT_MEMORY/`
...
CLAW_DISCORD_TOKEN=replace-with-your-journal-bot-token
...
8. `CLAW_CODEX_APPROVAL_POLICY=never` keeps the journal knowledge-base flow smooth instead of stopping for extra approval prompts.
